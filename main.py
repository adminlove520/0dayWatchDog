...
import threatbook
import cisa
import threatbook_premium
from poc_fetcher import fetch_poc_urls, set_github_token, load_github_token_from_config
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.src_combo.addItems(["ThreatBook", "GitHub", "ThreatBook API"])
    ...
    def _on_src_change(self):
        ...
        if src == "GitHub":
            hint = "粘贴 GitHub Token (PAT)"
        elif src == "ThreatBook API":
            hint = "粘贴微步在线API密钥"
        else:
            hint = "粘贴整串 Cookie"
    ...
    def apply_auth(self):
        ...
        if src == "ThreatBook":
            ...
            cfg = load_cfg()
            if txt:
                cfg["threatbook_api_key"] = txt
            else:
                cfg.pop("threatbook_api_key", None)
            save_cfg(cfg)
        else:  # GitHub
    ...
    def show_detail(self, row: int, _col: int):
        ...
        def premium_worker():
            # 检查是否配置了API密钥
            cfg = load_cfg()
            if not cfg.get("threatbook_api_key"):
                return
            ...
            try:
                # 查询漏洞详情
                vuln_data = threatbook_premium.query_vulnerability(item.cve)
                if not vuln_data or token != self._click_token:
                    return
                ...
                # CVSS信息
                if cvss_info := vuln_data.get("cvss"):
                    score = cvss_info.get("score", "")
                    vector = cvss_info.get("vector", "")
                    if score or vector:
                        html_parts.append(f"<br><b>[CVSS评估]</b><br>评分: {score}<br>向量: {vector}")
                
                # 影响范围
                affected_vendors = vuln_data.get("affected_vendors", [])
                if affected_vendors:
                    html_parts.append("<br><b>[影响范围]</b>")
                    for vendor in affected_vendors[:3]:  # 限制显示数量
                        vendor_name = vendor.get("name", "")
                        products = vendor.get("products", [])
                        for product in products[:2]:  # 每个厂商最多显示2个产品
                            product_name = product.get("name", "")
                            versions = product.get("versions", [])
                            version_str = ", ".join(versions[:3]) if versions else "所有版本"
                            html_parts.append(f"{vendor_name} {product_name} ({version_str})<br>")
                
                # 漏洞排查（公开POC）
                if pocs := vuln_data.get("pocs", []):
                    poc_urls = [poc.get("url") for poc in pocs if poc.get("url")]
                    if poc_urls:
                        links = "<br>".join(f'<a href="{u}">{u}</a>' for u in poc_urls[:3])
                        html_parts.append(f"<br><b>[漏洞排查]</b><br>{links}")
                
                # 修复方案
                if solution := vuln_data.get("solution"):
                    html_parts.append(f"<br><b>[官方修复方案]</b><br>{solution}")
                
                # 添加到详情页面
                if html_parts:
                    self.add_html.emit("<br><hr><b>[微步在线高级API]</b>" + "".join(html_parts))
            except Exception as exc:
                print("[ThreatBook Premium] error:", exc)
        
        threading.Thread(target=premium_worker, daemon=True).start()
